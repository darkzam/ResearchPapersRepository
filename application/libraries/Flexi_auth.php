...
class Flexi_auth extends Flexi_auth_lite
{
	...
	public function forgotten_password($identifier) 
	{
		...
		if ($this->CI->flexi_auth_model->forgotten_password($identity))
		{
			...
			// Set email data. aca modifique le quite todo lo de enviar email 
	 ...
	 * @author Rob Hussey
	 */
	public function forgotten_password_complete($user_id, $new_password = FALSE, $send_email = FALSE)
	{
		if (true)
		{
			$sql_select = array(
			...
			$new_password = $this->CI->flexi_auth_model->change_forgotten_password($user_id, $new_password, $database_salt);
	...
	public function update_user($user_id, $user_data)
	{
		...
		$this->CI->flexi_auth_model->set_error_message('form_validation_duplicate_identity', 'config');
		return FALSE;
	}
