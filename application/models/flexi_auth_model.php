...
class Flexi_auth_model extends Flexi_auth_lite_model
{
	...
	{ 
	    ...
	    $sql_insert = array(
			...
			$this->auth->tbl_col_user_account['suspend'] => $suspend_account
                    //aqui modifique
                  //  , 'uacc_identificacion' => $custom_data['upro_codigo'] 
		);
	 ...
	 * @author Rob Hussey
	 */
	public function change_forgotten_password($user_id, $new_password = FALSE, $database_salt = FALSE)
	{
	    // Confirm token is 40 characters long (length of sha1)
		if (!is_numeric($user_id)) 
	    {
	...
	{	
		...
		//aca puede estar el error logico
				...
				if ($this->set_login_sessions($user, TRUE))
				{
					...
					else
					{
						$this->flexi_auth_lite_model->delete_remember_me_cookies();
                                       ...
                                       $this->usuario_model->insert_login($fecha);
					return TRUE;
				}
			}
